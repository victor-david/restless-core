...
class CoreView implements TranslatorAwareInterface
{
    ...
    /**
     * Gets or sets an integer value that determines the level of debug mode.
     *
     * @var int
     */
    public $debug = 0;
    ...
    /**
     * Gets or sets an integer value that determines if the html output will be minified
     *
     * @var int
     */
    public $minifyHtml = 0;
    ...
    public function includeFile(string $key, string $file): void
    {
        if (!$key || !$file)
        {
            ViewException::throwViewException('Include: Invalid parameter(s)');
        }
    ...
    /**
     * Outputs the parsed template using echo.
     */
    ...
    /**
     * Gets the output of the parsed template without outputing it.
     *
     * @return string
     */
    ...
    private function mergeConditionals(&$s)
    {
        $out= [];
        /* no matches, no cry */
        if (!preg_match_all($this->rxCondition, $s, $out, PREG_OFFSET_CAPTURE))
        {
            return;
        }
    ...
    private function mergeLoops(&$s)
    {
        $out= [];
        /* no matches, no cry */
        if (!preg_match_all($this->rxLoop, $s, $out, PREG_OFFSET_CAPTURE))
        {
            return;
        }
    ...
    private function mergeLanguage(&$s)
    {
        $out = [];
        /* no matches, no cry */
        if (!preg_match_all($this->rxLanguage, $s, $out))
        {
            return;
        }
    ...
    private function mergeUserObjects(&$s)
    {
        $out = [];
        /* no matches, no cry */
        if (!preg_match_all($this->rxUserObj, $s, $out))
        {
            return;
        }
