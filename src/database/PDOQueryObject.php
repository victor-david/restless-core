...
class PDOQueryObject
{
   ...
   * @param string $table (for self::COMMAND) is command string like 'SHOW TABLE STATUS'
  ...
  */
  ...
  {
    if ($this->type != self::SELECT) throw new Exception('Type must be select [join]');
  ...
  */
  ...
  {
    if (count($this->join) == 0) return null;
